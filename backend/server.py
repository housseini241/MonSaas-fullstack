...
# ---------- Models (importés depuis models.py) ----------
from models import (
    UserPublic,
    RegisterIn,
    LoginIn,
...
async def stripe_webhook(request: Request):
    ...
    try:
        ...
        if event.session_id:
            ...
            if domain_purchase:
                await apply_domain_purchase_if_paid(db, event.session_id, _stripe_deps)
...
# Dépendances partagées injectées dans shop_routes (Stripe / email)
_stripe_deps = {
...
async def api_create_shop(body: ShopCreate, user: dict = Depends(current_user)):
    return await create_shop(db, body, user, is_pro)


@api_router.get("/shops")
async def api_list_shops(user: dict = Depends(current_user)):
    return await list_shops(db, user)


@api_router.get("/shops/{shop_id}")
async def api_get_shop(shop_id: str, user: dict = Depends(current_user)):
    return await get_shop(db, shop_id, user)


@api_router.put("/shops/{shop_id}")
async def api_update_shop(shop_id: str, body: ShopUpdate, user: dict = Depends(current_user)):
    return await update_shop(db, shop_id, body, user)


@api_router.post("/shops/{shop_id}/publish")
async def api_publish_shop(shop_id: str, user: dict = Depends(current_user)):
    return await publish_shop(db, shop_id, user)


@api_router.delete("/shops/{shop_id}")
async def api_delete_shop(shop_id: str, user: dict = Depends(current_user)):
    return await delete_shop(db, shop_id, user)


@api_router.post("/shops/{shop_id}/products")
async def api_create_product(shop_id: str, body: ProductIn, user: dict = Depends(current_user)):
    return await create_product(db, shop_id, body, user)


@api_router.get("/shops/{shop_id}/products")
async def api_list_products(shop_id: str, user: dict = Depends(current_user)):
    return await list_products(db, shop_id, user)


@api_router.put("/shops/{shop_id}/products/{product_id}")
async def api_update_product(shop_id: str, product_id: str, body: ProductUpdate, user: dict = Depends(current_user)):
    return await update_product(db, shop_id, product_id, body, user)


@api_router.delete("/shops/{shop_id}/products/{product_id}")
async def api_delete_product(shop_id: str, product_id: str, user: dict = Depends(current_user)):
    return await delete_product(db, shop_id, product_id, user)


@api_router.post("/shops/{shop_id}/upload-image")
async def api_upload_shop_image(shop_id: str, file: UploadFile = File(...), kind: str = Form("product"), user: dict = Depends(current_user)):
    return await upload_shop_image(db, shop_id, file, kind, user, upload_image_bytes)


@api_router.get("/public/shops/{slug}")
async def api_public_shop(slug: str):
    return await public_shop(db, slug)


@api_router.get("/public/shops/{slug}/products/{product_slug}")
async def api_public_product(slug: str, product_slug: str):
    return await public_product(db, slug, product_slug)


@api_router.post("/public/shops/{slug}/checkout")
async def api_shop_checkout(slug: str, body: CheckoutShopIn):
    return await shop_checkout(db, slug, body, _stripe_deps)


@api_router.get("/public/shops/{slug}/orders/status/{session_id}")
async def api_public_order_status(slug: str, session_id: str):
    return await public_order_status(db, slug, session_id, _stripe_deps, _email_deps)


@api_router.get("/shops/{shop_id}/orders")
async def api_list_orders(shop_id: str, user: dict = Depends(current_user)):
    return await list_orders(db, shop_id, user)


@api_router.put("/shops/{shop_id}/orders/{order_id}")
async def api_update_order_status(shop_id: str, order_id: str, body: OrderStatusUpdate, user: dict = Depends(current_user)):
    return await update_order_status(db, shop_id, order_id, body, user)
...
from domain_routes import (
    ...
    list_domains,
    domain_status,
    domain_connect_to_project,
    apply_domain_purchase_if_paid,
    tld_price,
    set_platform_a_record,
)
from models import DomainPurchaseIn

# Injecte la valeur PLATFORM_A_RECORD (lue depuis l'environnement) dans domain_routes
set_platform_a_record(os.environ.get("PLATFORM_A_RECORD_IP", "76.76.21.21"))


@api_router.get("/domains/search")
async def api_domain_search(name: str, business_type: Optional[str] = None, city: Optional[str] = None, _user: dict = Depends(current_user)):
    return await domain_search(db, name, business_type, city)


@api_router.post("/domains/purchase")
async def api_domain_purchase(body: DomainPurchaseIn, user: dict = Depends(current_user)):
    return await domain_purchase(db, body, user, _stripe_deps)


@api_router.get("/domains")
async def api_list_domains(user: dict = Depends(current_user)):
    return await list_domains(db, user)


@api_router.get("/domains/status/{session_id}")
async def api_domain_status(session_id: str, user: dict = Depends(current_user)):
    return await domain_status(db, session_id, user, _stripe_deps)


@api_router.post("/domains/{domain_id}/connect")
async def api_domain_connect_to_project(domain_id: str, body: Dict[str, Any], user: dict = Depends(current_user)):
    return await domain_connect_to_project(db, domain_id, body, user)
